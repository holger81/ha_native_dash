...
def collect_entities(node, acc: set[str]) -> None:
    if isinstance(node, dict):
        for k, v in node.items():
            if k in {"entity", "activity_entity", "battery", "home_sensor", "work_sensor", "graph", "poster", "weather_entity", "temp_entity", "hum_entity", "climate_entity"} and isinstance(v, str) and "." in v:
                acc.add(v)
